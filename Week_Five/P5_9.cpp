...
int main() 
{
	...
	// print the header
	cout << setw(10) << monthsName << " " << year << endl;

	// prints the Weekdays using a for loop
	for (int i = 0; i <= 6; i++) {
		if (i == 0) {
			cout << "Su ";
		}
	...
	// Print the white spaces before the first day is printed
	for (int i = 0; i < dayWeek; i++) {
		cout << "   ";
	}

	// Prints the days of the week, with a counter to make a new line
	int counter = dayWeek;	
	for (int j = dayWeek; j <=daysInMonth; j++) {
		counter++;
